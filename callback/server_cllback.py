...
def upload_weights(client_id):
    ...
    if len(uploaded_files) == n:
        ...
        # Notify clients about the availability of global weights
        notify_clients()
...
def notify_clients():
    """Send a notification to all clients about the availability of global weights."""
    client_urls = [
        "http://localhost:5001/api/notify_global_weights",
        "http://localhost:5002/api/notify_global_weights",
    ]
    ...
    for url in client_urls:
        try:
            ...
            print(f"Failed to notify client at {url}: {e}")
