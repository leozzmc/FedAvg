...
def main():
    ...
    for model_id in range(modelcount):
        ...
        # Load global weights into the model and retrain
        for i in range(modelcount):
            model = YOLO('yolov8n.pt')  # Load a fresh model instance
            print(f"Loading global weights into Model {i}...\n")
            global_weights = torch.load(global_weights_file)
            model.load_state_dict(global_weights)

            # Retrain with the updated global weights
            print(f"Retraining Model {i}...\n")
            fedclient.train_on_client(model, datasets[i], epochs_client, batch_size, client_id)

            # Evaluate the updated model
            local_eval = fedclient.evaluate_model(model, datasets[i], client_id)
            if 'accuracy' in local_eval:
                ...
                print(f"Client {client_id} - Model {i} Local evaluation results: {local_eval}")
